...
public class RPG {
    ...
    // ==================== CAPÍTULO 5 ====================
    private void aplicarPerguntaSwitchCase() {
        boolean acertou = false;
...
""");
            ...
            pausar(1000);
            System.out.println("Pergunta: Qual saída será exibida?");
            System.out.println("a) Grupo 1");
            System.out.println("b) Grupo 2");
            System.out.println("c) Grupo 3");
            System.out.println("d) Grupo Desconhecido");
            System.out.print("Resposta: ");
            String resposta = sc.next();

            if (resposta.equalsIgnoreCase("d")) {
                System.out.println("\nCorreto! O loop se quebra e uma saída aparece!");
                conhecimento += 3;
                acertou = true;
            } else {
                int dano = (int) (Math.random() * 2) + 1;
                vida -= dano;
                System.out.println("\nErrado! O túnel reinicia ao seu redor...");
                System.out.println("Você sofre " + dano + " de dano!");
                pausar(1200);
            }
        }
    }
    ...
    private void capitulo5() {
        ...
        switch (escolha) {
            case 1 -> {
                ...
                pausar(1500);
                System.out.println("As paredes exibem códigos brilhantes...");
                pausar(1500);
                System.out.println("Você percebe que precisa resolver o enigma para escapar!");
                pausar(1500);

                aplicarPerguntaSwitchCase();
            }

            case 2 -> {
                System.out.println("\nVocê tenta observar o ciclo e entender seu significado...");
                pausar(1500);
                System.out.println("Mas nada acontece. O loop continua igual.");
                pausar(1500);
                System.out.println("Parece que você precisa resolver o enigma para escapar.");
                pausar(1500);
            ...
            default -> {
                System.out.println("\nVocê hesita e o túnel percebe sua indecisão...");
                pausar(2000);
                System.out.println("O túnel se fecha ao seu redor e o loop se torna verdadeiramente infinito.");
                pausar(2000);
                System.out.println("Você ficará preso aqui para sempre...");
                pausar(2000);
                System.out.println("\n=== GAME OVER ===");
                vida = 0;
                return;
    ...
    // ==================== CAPÍTULO 7 ====================
            private void capituloSete() {
                System.out.println("\n=== CAPÍTULO 7 – O FIREWALL DA VERDADE ===");
                pausar(1200);
                System.out.println("Você chega a um enorme muro de energia, vibrando entre código puro e código corrompido.");
                pausar(1500);
                System.out.println("No centro, uma inscrição brilha:");
                pausar(1500);
                System.out.println("O que é um erro: o bug ou quem o criou?");
                pausar(2000);

                System.out.println("\nComo você responde?");
                System.out.println("1 - O erro é o bug.");
                System.out.println("2 - O erro é o criador.");
                System.out.print("Escolha: ");
                int escolha = sc.nextInt();
                pausar(1000);

                if (escolha == 1) {
                    System.out.println("\nO firewall permite sua passagem, mas registra seu medo...");
                    conhecimento += 1;
                } else if (escolha == 2) {
                    System.out.println("\nO firewall se abre com respeito, reconhecendo sua responsabilidade.");
                    conhecimento += 3;
                } else {
                    int dano = (int) (Math.random() * 3) + 1;
                    vida -= dano;
                    System.out.println("\nVocê hesita. O firewall oscila e te atinge. -" + dano + " de vida.");
                }
    ...
    // ==================== CAPÍTULO 8 ====================
    private void capituloOito() {
        System.out.println("\n=== CAPÍTULO 8 – O SERVIDOR FANTASMA ===");
        pausar(1200);
        System.out.println("Nos confins de System32, você avança por processos esquecidos...");
        pausar(1500);
        System.out.println("Arquivos antigos e diretórios quebrados ecoam ao seu redor.");
        pausar(1500);
        System.out.println("Então, um programa antigo surge flutuando:");
        pausar(1200);
        System.out.println("Clippy.exe aparece!!!");
        pausar(2000);

        System.out.println("Ele te faz uma pergunta!");
        System.out.println("""
int x = 4, y = 9, z = 7;
...
""");
        ...
        pausar(1000);
        System.out.println("Pergunta: O que será impresso?");
        System.out.println("a) Crescente");
        System.out.println("b) Meio Igual");
        System.out.println("c) Desordenado"); // correta
        System.out.println("d) Decrescente");
        String resposta = sc.next();

        if (resposta.equalsIgnoreCase("c")) {
            System.out.println("Certo! A resposta correta é c) Desordenado.");
            System.out.println("\nClippy.exe decide seguir você.");
            pausar(1500);
            System.out.println("Ele revela atalhos ocultos e códigos que facilitam sua jornada.");
            conhecimento += 3;
        } else {
            int dano = (int) (Math.random() * 3) + 1;
            vida -= dano;
            ...
            pausar(1500);
            System.out.println("\nClippy dá uma risada ensurdecedora e diz:");
            System.out.println("\"O caminho agora é mais difícil, cheio de erros e exceções inesperadas!\"");
            pausar(1500);
            System.out.println("Ele sai voando para longe...");
            pausar(1500);
            System.out.println("Você sente uma forte dor no ouvido após a risada de Clippy.");

            System.out.println("Você sofre " + dano + " de dano, mas aprende a lidar com falhas por conta própria.");
            conhecimento -= 2;
        }
